...
read = TT.dot(result, M)
weight_dim = result.reshape((weight.shape[1],)).dimshuffle((0, 'x'))
erase_dim = erase.reshape((erase.shape[0],)).dimshuffle(('x', 0))
add_dim = add.reshape((add.shape[0],)).dimshuffle(('x', 0))
memory_erase = M*(1-(weight_dim*erase_dim))
memory = memory_erase+(weight_dim*add_dim)
...
attfunc = theano.function(inputs=[M,oldstate, w_state,w_memory,w_outer], outputs=[m_vec,s_vec,inner,innert,ot,otexp,result,read,memory_erase,memory])
wbt = numpy.asarray([0.3,.7], dtype=theano.config.floatX)
kt = numpy.asarray([1.,.5,.2], dtype=theano.config.floatX)
Mt = numpy.asarray([[1,1.6,2.1],[.1,.1,-.1]], dtype=theano.config.floatX)
...
eraset = numpy.asarray([.3,.5,.7], dtype=theano.config.floatX)
addt = numpy.asarray([3,.5,3], dtype=theano.config.floatX)
#print simfunc(wbt,kt, Mt,betat,gt,simt,oldt, oldbt,eraset,addt)
ost = numpy.asarray([0.4,.5], dtype=theano.config.floatX)
...
result = otexp/normalizer
read = TT.sum(result.dimshuffle(0,1,'x')*M,axis=1)
M_erase = M*(1-weight.dimshuffle(0,1,'x')*erase.dimshuffle(0,'x',1))
memory = M_erase+weight.dimshuffle(0,1,'x')*add.dimshuffle(0,'x',1)
#result = weight.dimshuffle(0,1,'x')*add.dimshuffle(0,'x',1)
...
attfunc = theano.function(inputs=[M,oldstate, w_state,w_memory,w_outer], outputs=[m_vec,s_vec,inner,innert,ot,otexp,normalizer,result,read,M_erase,memory])
print attfunc(Mt,ost,wst,wmt,outert)
