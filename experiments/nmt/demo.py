...
class BeamSearch(object):
    ...
    def __init__(self, enc_dec, rnnsearch):
        self.enc_dec = enc_dec
        state = self.enc_dec.state
        self.rnnsearch = rnnsearch
        state2 = self.rnnsearch.state
    ...
    def compile(self):
        ...
        self.comp_repr2 = self.rnnsearch.create_representation_computer()
        self.comp_init_states2 = self.rnnsearch.create_initializers()
        self.comp_next_probs2 = self.rnnsearch.create_next_probs_computer()
        self.comp_next_states2 = self.rnnsearch.create_next_states_computer()
    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1):
        ...
        c2 = self.comp_repr2(seq)[0]
        states2 = map(lambda x : x[None, :], self.comp_init_states2(c2))
        dim2 = states2[0].shape[1]
        ...
        for k in range(3 * len(seq)):
            ...
            log_probs1 = numpy.log(self.comp_next_probs(c, k, last_words, mem, *states)[0])
            log_probs2 = numpy.log(self.comp_next_probs2(c2, k, last_words, *states2)[0])
            ...
            new_states = [numpy.zeros((n_samples, dim), dtype="float32") for level
                    in range(num_levels)]
            new_states2 = [numpy.zeros((n_samples, dim2), dtype="float32") for level
                    in range(num_levels)]
            ...
            for i, (orig_idx, next_word, next_cost) in enumerate(
                ...
                for level in range(num_levels):
                    ...
                    new_states2[level][i] = states2[level][orig_idx]
                inputs[i] = next_word
                new_mem[i] = mem[orig_idx]
            result = self.comp_next_states(c, k, inputs, new_mem,*new_states)
            new_states =[result[0]]
            new_mem = result[1]
            new_states2 = self.comp_next_states2(c2, k, inputs, *new_states2)
            ...
            states2 = map(lambda x : x[indices], new_states2)
...
def main():
    ...
    state2 = prototype_state()
    with open(args.state2) as src:
        state2.update(cPickle.load(src))
    state2.update(eval("dict({})".format(args.changes)))

    logging.basicConfig(level=getattr(logging, state['level']), format="%(asctime)s: %(name)s: %(levelname)s: %(message)s")

    rng = numpy.random.RandomState(state['seed'])
    rnnsearch = RNNEncoderDecoder(state2, rng, skip_init=True)
    rnnsearch.build()
    enc_dec = NTMEncoderDecoder(state, rng, skip_init=True)
    enc_dec.build()
    lm_model = enc_dec.create_lm_model()
    lm_model.load(args.model_path)
    lm_model2 = rnnsearch.create_lm_model()
    lm_model2.load(args.model_path2)
    indx_word = cPickle.load(open(state['word_indx'],'rb'))

    sampler = None
    beam_search = None
    if args.beam_search:
        beam_search = BeamSearch(enc_dec, rnnsearch)
        beam_search.compile()
    else:
        sampler = enc_dec.create_sampler(many_samples=True)

    idict_src = cPickle.load(open(state['indx_word'],'r'))
    return beam_search, idict_src
...
@app.route('/translateapi/<sentence>', methods=['GET', 'POST'])
def translateapi(sentence):
    sentence = sentence.encode('utf-8')
    seq, parsed_in = parse_input(state, indx_word, sentence, idx2word=idict_src)
    print "Parsed Input:", parsed_in
    trans, costs, _ = sample(lm_model, seq, n_samples, sampler=sampler,
                    beam_search=beam_search, ignore_unk=args.ignore_unk, normalize=args.normalize)
    if len(trans) == 0:
