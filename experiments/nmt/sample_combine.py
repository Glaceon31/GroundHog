...
class BeamSearch(object):
    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1, maxsearchlen=30):
        if len(seq) > maxsearchlen:
            ...
            c = self.comp_repr2(seq)[0]
            states = map(lambda x : x[None, :], self.comp_init_states2(c))
            dim = states[0].shape[1]
            ...
            for k in range(3 * len(seq)):
                ...
                log_probs = numpy.log(self.comp_next_probs2(c, k, last_words, *states)[0])
                ...
                for i, (orig_idx, next_word, next_cost) in enumerate(
                    ...
                    for level in range(num_levels):
                        new_states[level][i] = states[level][orig_idx]
                    inputs[i] = next_word
                new_states = self.comp_next_states2(c, k, inputs, *new_states)
